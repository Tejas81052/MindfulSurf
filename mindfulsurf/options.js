...
const SETTINGS_KEY = "msSettings"; // { showSnoozeCountdown?: boolean, devMode?: boolean }

// Mirror defaults used by background/popup so Options reflects effective limits
const DEFAULT_SITES = {
  "youtube.com": 10,
  "instagram.com": 5,
  "snapchat.com": 5,
...
function loadSettings() {
  const checkbox = document.getElementById("toggleSnoozeCountdown");
  if (!checkbox) return;
  chrome.storage.sync.get([SETTINGS_KEY], (data) => {
    const s = data[SETTINGS_KEY] || {};
    checkbox.checked = !!s.showSnoozeCountdown;
  });
  if (!checkbox._wired) {
    ...
    checkbox.addEventListener("change", () => {
      chrome.storage.sync.get([SETTINGS_KEY], (data) => {
        const s = data[SETTINGS_KEY] || {};
        s.showSnoozeCountdown = !!checkbox.checked;
...
function getUrlDevFlag() {
  try {
    const u = new URL(window.location.href);
    return u.searchParams.get("dev") === "1" || u.hash.includes("dev=1");
  } catch (e) {
    return false;
  }
}
...
function loadDevSection() {
  ...
  const showFromUrl = getUrlDevFlag();
  chrome.storage.sync.get([SETTINGS_KEY], (data) => {
    const s = data[SETTINGS_KEY] || {};
    const show = !!s.devMode || showFromUrl;
  ...
  if (!btn._wired) {
    ...
    btn.addEventListener("click", () => {
      if (!confirm("Reset today's usage and snooze flags?")) return;
      const today = new Date().toISOString().slice(0, 10);
      chrome.storage.local.set(
        { siteUsage: {}, lastResetDay: today, snoozeUsed: {} },
        () => {
          try {
            chrome.runtime.sendMessage({ type: "ms_enforce" });
          } catch (e) {}
          loadUsage();
        }
      );
